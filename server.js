...
function parseVlessLink(vless) {
  // Supports: vless://UUID@HOST:PORT?flow=...&security=reality&sni=...&fp=...&pbk=...&sid=...
  const s = String(vless || '').trim();
  if (!s.startsWith('vless://')) throw new Error('Not a vless:// link');

  // URL can choke on some chars in fragment; still generally ok.
  const u = new URL(s);

  const uuid = decodeURIComponent(u.username || '').trim();
  const server = (u.hostname || '').trim();
  const server_port = u.port ? Number(u.port) : null;

  if (!uuid || !server || !server_port) throw new Error('vless link missing uuid/host/port');

  const q = u.searchParams;
  const flow = q.get('flow') || undefined;

  // reality/tls info
  const security = (q.get('security') || '').toLowerCase(); // "reality" or "tls" etc
  ...
  // Only set tls if link indicates it
  if (security === 'reality' || security === 'tls') {
    ...
    if (security === 'reality') {
      patch.tls.reality = { enabled: true };
      if (pbk) patch.tls.reality.public_key = pbk;
...
app.put('/sb/api/vless', async (req, res) => {
  ...
  const patch = parseVlessLink(vless);
